...
function render() { const hash = location.hash; if (!user) app.innerHTML = hash === '#login' ? authForm('login') : hash === '#register' ? authForm('register') : hash.startsWith('#scan=') ? `${nav()}<main class="shell"><div class="empty">Please log in first, then scan the QR card again.</div></main>` : hero(); else if (hash.startsWith('#game=')) app.innerHTML = game ? gameView() : `${nav()}<main class="shell"><div class="empty">Loading board…</div></main>`; else if (hash.startsWith('#scan=')) app.innerHTML = `${nav()}<main class="shell"><div class="empty">Scanning…</div></main>`; else app.innerHTML = lobby(); }
