...
function lobby() { return `${nav()}<main class="shell"><section class="lobby-head"><div><div class="eyebrow">Your arena</div><h1>Find your next match.</h1></div><button class="button gold" data-action="new-game">+ Create a game</button></section><section class="games">${games.length ? games.map((item) => `<article class="game-card" data-action="join" data-id="${item.id}" role="button" tabindex="0"><div class="game-meta"><span class="pill">${item.status === 'waiting' ? 'awaiting rival' : 'in play'}</span><span>${item.id}</span></div><h3>${escape(item.title)}</h3><p>Hosted by ${escape(item.owner_name)} · ${item.player_count}/2 players</p><button class="button ${item.status === 'waiting' ? 'gold' : ''}" tabindex="-1">${item.status === 'waiting' ? 'Join game' : 'Watch board'}</button></article>`).join('') : '<div class="empty">No open games yet. Create the first board.</div>'}</section></main>`; }
function newGameModal() { return `<div class="new-game"><form class="modal" id="new-game-form"><div class="eyebrow">New table</div><h2>Set the board.</h2><p style="font-size:13px;color:#617066;line-height:1.6">You play White. Your opponent will join as Black.</p><div class="field"><label>Game title</label><input required name="title" minlength="3" maxlength="40" value="Dictionary Duel" /></div><div class="modal-actions"><button type="button" class="button ghost" data-action="close-modal">Cancel</button><button class="button">Create game</button></div></form></div>`; }
...
function isMyTurn() { const mine = myPlayer(); return mine && !game.challenge && (game.fen.split(' ')[1] === (mine.color === 'white' ? 'w' : 'b')); }
function gameBoard() {
...
function sidePanel() { const active = game.fen.split(' ')[1] === 'w' ? 'White' : 'Black'; const recent = game.moves.slice(-12).reverse(); return `<aside class="side-panel"><section class="panel turn"><div class="eyebrow" style="color:#a7d39f">${game.status === 'finished' ? 'game complete' : game.challenge ? 'capture challenge open' : isMyTurn() ? 'your move' : 'opponent is thinking'}</div><h3 style="font-size:19px;margin:7px 0">${game.status === 'finished' ? 'Checkmate. King taken.' : game.challenge ? `${labels[game.challenge.capturedPiece]} challenge` : `${active} to move`}</h3><p>${game.challenge ? (ownsChallenge() ? 'Answer correctly to earn an immediate extra move.' : 'Play is paused while the capturer answers.') : 'Every capture tests your dictionary mastery.'}</p></section><section class="panel"><h3>Players & mastery</h3>${game.players.map((player) => `<div class="player"><span class="player-dot ${player.color}"></span><span class="player-name">${escape(player.username)} ${player.id === user.id ? '(you)' : ''}</span><span class="score">${player.points} XP</span></div>`).join('')}</section><section class="panel"><h3>Move log</h3><div class="moves">${recent.length ? recent.map((move, index) => `<span class="move">${game.moves.length - index}. ${escape(move.san)}</span>`).join('') : '<span>First move awaits.</span>'}</div></section>${qr ? `<section class="panel qr"><img src="${qr.image}" alt="Game QR code" /><p><strong>Physical board QR</strong><br/>Print this QR beside the physical board. It opens this live match and its challenge station.</p></section>` : ''}</aside>`; }
function gameView() { return `${nav()}<main class="shell"><section class="game-layout"><div><div class="game-title"><div><div class="eyebrow">Table ${game.id}</div><h1>${escape(game.title)}</h1></div><button class="button ghost" data-action="back">Arena</button></div>${gameBoard()}</div>${sidePanel()}</section></main>${game.challenge && ownsChallenge() ? challengeModal() : ''}${answerResult ? resultModal() : ''}`; }
...
function render() { const hash = location.hash; if (!user) app.innerHTML = hash === '#login' ? authForm('login') : hash === '#register' ? authForm('register') : hero(); else if (hash.startsWith('#game=')) app.innerHTML = game ? gameView() : `${nav()}<main class="shell"><div class="empty">Loading board…</div></main>`; else app.innerHTML = lobby(); }

async function loadGames() { games = await api('/games'); render(); }
async function openGame(id) { game = await api(`/games/${id}/join`, { method: 'POST' }); location.hash = `#game=${id}`; connectSocket(); socket.emit('game:join', { gameId: id }); selected = null; qr = null; challengeQuestion = null; answerResult = null; render(); try { qr = await api(`/games/${id}/qr`); render(); } catch {} }
function connectSocket() { if (socket?.connected) return; socket = io({ auth: { token } }); socket.on('game:state', async (nextGame) => { if (!game || nextGame.id !== game.id) return; game = nextGame; selected = null; if (ownsChallenge() && !challengeQuestion && !answerResult) { try { challengeQuestion = await api(`/games/${game.id}/question`); } catch (error) { notice(error.message); } } if (!game.challenge) challengeQuestion = null; render(); }); socket.on('challenge:earned', (payload) => notice(`${labels[payload.capturedPiece]} captured — ${payload.difficulty} challenge unlocked.`)); socket.on('app:error', notice); socket.on('connect_error', () => notice('Live connection unavailable. Refresh to reconnect.')); }
